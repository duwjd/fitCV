...
export async function getApplicationPipeline() {
  ...
  try {
    ...
    console.error("지원 파이프라인 조회 실패:", error);
    return [];
  }
}
...
export async function getCareerGoals() {
  ...
  try {
    return await prisma.careerGoal.findMany({
      where: { userId },
...
export async function toggleCareerGoal(goalId: string) {
  ...
  try {
    ...
    console.error("커리어 목표 토글 실패:", error);
    return { success: false as const, error: "목표 상태 변경에 실패했습니다." };
  }
}
